    ...
    <div class="relative flex-1 flex items-center justify-center py-10">
      <div class="w-full max-w-[440px] stagger">
        ...
        <h1 class="font-display font-bold text-[40px] sm:text-[48px] leading-[1.02] text-ink mb-3">
          Welcome <br/>back, builder.
        </h1>
        <p class="text-ink/70 text-[15px] mb-7 max-w-[380px]">
          Log in to ship today's tasks. Your team is already moving.
        </p>

        @if (session('status'))
          <div class="mb-4 px-4 py-3 border-[3px] border-ink rounded-[10px] bg-lime-accent/40 font-mono text-[12px] font-semibold">
            {{ session('status') }}
          </div>
        @endif
        ...
        <div class="seg-track mb-6">
          ...
          <a class="seg-btn active" href="{{ route('login') }}">Sign in</a>
          @if (Route::has('register'))
            <a class="seg-btn" href="{{ route('register') }}">Sign up</a>
          @else
            <span class="seg-btn">Sign up</span>
          @endif
        </div>
        ...
        <form method="POST" action="{{ route('login') }}" novalidate>
          ...
          <div class="mb-2">
            <div class="flex items-end justify-between mb-2">
              <label for="password" class="font-display font-semibold text-[12px] uppercase tracking-[0.14em] text-ink">Password</label>
              @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="font-mono text-[11px] font-semibold text-electric-blue hover:underline underline-offset-4 decoration-2">
                  Forgot password?
                </a>
              @endif
            </div>
            ...
            @error('password')
              <p class="mt-2 text-red-accent text-[13px] font-semibold">{{ $message }}</p>
            @enderror
          </div>
          ...
          <div class="flex items-center justify-between mt-4 mb-6">
            <label for="remember_me" class="flex items-center gap-2.5 cursor-pointer select-none">
              <input id="remember_me" type="checkbox" name="remember" class="brutal-check" />
              <span class="text-[14px] font-medium text-ink">Remember me on this device</span>
            </label>
          </div>
