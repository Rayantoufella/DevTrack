...
<title>DevTrack — Modifier {{ $project->title }}</title>
  ...
  <main class="flex-1 min-w-0 relative">
    ...
    <div class="main-pad relative px-8 py-10 stagger">
      <div class="max-w-[680px] mx-auto">
        ...
        <div class="mb-6">
          <div class="font-mono text-[11px] font-semibold tracking-[0.18em] text-electric-blue uppercase mb-2">// editing project</div>
          <h1 class="font-display font-bold text-[36px] sm:text-[44px] leading-[1] tracking-tight">Edit project details.</h1>
        </div>
        ...
        <div class="form-card">
          <form method="POST" action="{{ route('projects.update', $project) }}">
            ...
            <div class="px-7 py-6 border-b-[3px] border-ink flex items-center gap-4" style="background:#EFF6FF;">
              ...
              <div>
                <h2 class="font-display font-bold text-[22px] tracking-tight leading-tight">{{ $project->title }}</h2>
                <p class="text-[12px] text-ink/60 font-mono mt-0.5">PROJ-{{ str_pad($project->id, 2, '0', STR_PAD_LEFT) }} · créé le {{ Carbon::parse($project->created_at)->format('d/m/Y') }}</p>
              </div>
            </div>
            ...
            <div class="px-7 py-6 space-y-6">
              <div>
                ...
                <input id="title" name="title" type="text" value="{{ old('title', $project->title) }}" maxlength="255"
              ...
              <div>
                ...
                <textarea id="description" name="description" rows="4" required
                          class="form-input resize-none @error('description') err @enderror">{{ old('description', $project->description) }}</textarea>
                @error('description')
                  <p class="form-err">
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" x2="12" y1="8" y2="12"/><line x1="12" x2="12.01" y1="16" y2="16"/></svg>
                    {{ $message }}
              ...
              <div>
                ...
                <input id="deadline" name="deadline" type="date"
                       value="{{ old('deadline', $deadline->toDateString()) }}" required
              ...
              @can('delete', $project)
                <div class="border-[3px] border-red-accent rounded-[10px] p-4" style="background:#FEF2F2;">
                  <div class="flex items-start justify-between gap-3 flex-wrap">
                    ...
                    <button type="submit" form="form-archive" class="btn-mini danger">
          ...
          @can('delete', $project)
            {{-- Formulaire séparé pour le bouton « Archiver » (évite les formulaires imbriqués) --}}
            <form id="form-archive" method="POST" action="{{ route('projects.destroy', $project) }}"
